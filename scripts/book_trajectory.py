...
# tsne_articles_2D = tsne_model_2D.fit_transform(par_vecs)
#
...
heatmap = np.matrix(np.array(norm_vecs))
plt.imshow(heatmap, interpolation='nearest')
plt.savefig('../figures/book_trajectories/'+(b.split('/')[4])[0:-4]+'_heatmap_norm_par_'+str(int(par_length/1000))+'k.pdf')
